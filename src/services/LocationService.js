...
export const updateLocation = async (id, description, address, latitude, longitude, formData) => {
    try {
        ...
        const response = await axios.put(`${BASE_API_URL}/location/update-location?locationId=${id}&description=${description}&address=${address}&latitude=${latitude}&longitude=${longitude}`, formData,
           );
        console.log(response.data);
        console.log(response.status);
